...
export const ffmpegClient = async (file:Ifiles, filedata:Filedata ,conversionexttype:FileconversionType , convesionURLType:FileURLType) =>{
    ...
    await ffmpeg.load({
        coreURL: await toBlobURL(`${String(process.env.FFMPEG_BASEURL)}/ffmpeg-core.js`, 'text/javascript'),
        wasmURL: await toBlobURL(`${String(process.env.FFMPEG_BASEURL)}/ffmpeg-core.wasm`, 'application/wasm')
      });
      ...
      await writeFile(filePath , filedata);
       ...
       let convertURL = URL.createObjectURL(new Blob([data.buffer], { type: convesionURLType }))
